  ...
  test('getting a transaction from testnet', function(t) {
    common.setup(test, function(err, commonBlockchain) {
      commonBlockchain.Transactions.Get({txids: [txid]}, function(err, txs) {
        ...
        var vin = tx.vin[0];
        t.equal(vin.txid, txid, "tx.vin[0].txid should be txid")
        t.equal(vin.txId, txid, "tx.vin[0].txId should be txid")
        ...
        t.equal(op_return_output.index, 0, "op_return_output.index should be 0")
        t.equal(op_return_output.n, 0, "op_return_output.n should be 0")
        ...
        t.equal(change_output.index, 1, "change_output.index should be 1")
        t.equal(change_output.n, 1, "change_output.n should be 1")
        ...
        t.equal(change_output.spentTxid, '9e4a932d27851a254715b30742de3eca379cdb1ffdadb0f9896f2df90c44a72e', "change_output.spentTxid should be 9e4a932d27851a254715b30742de3eca379cdb1ffdadb0f9896f2df90c44a72e")
        t.end()
      });
    })
  })
